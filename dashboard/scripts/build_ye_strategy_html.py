...
def build_daily_page() -> str:
    ...
    readiness = json.loads((PROJECT / "results/live/readiness_report.json").read_text())
    # The report date comes from this run, never the last successful backtest.
    date = readiness["signal_date"]
    return render(collect(PROJECT, date), public=True)
...
def main() -> None:
    ...
    from etf_rotation.daily_report import read_json, publish
    report_date = read_json(PROJECT / "results/live/readiness_report.json")["signal_date"]
    OUTPUT_BACKTEST_HTML.write_text(with_output_links(backtest_html), encoding="utf-8")
    publish(PROJECT, report_date)
    print(PUBLIC_HTML)
