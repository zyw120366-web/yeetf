...
STRATEGY_SECTION = r'''<section class="section" id="strategy"><div class="section-title"><h2>策略规则：每日决策流程</h2><span>当天收盘决定目标仓位 · 下一交易日开盘执行</span></div><article class="card daily-intro"><h3>核心冠军优先，挑战者只补空档</h3><p>全池由45只核心ETF和6只挑战者组成。核心池独立形成排名、类别宽度与退出标尺，并拥有买入优先权。挑战者只有完整通过全部条件、且当天没有任何核心候选时才允许补位；一旦核心候选重新出现，挑战者下一交易日开盘让位。</p></article><div class="flow"><article class="flow-step"><b>收盘后 · 第 1 步</b><h3>冻结当天输入</h3><p>更新全部51只ETF的日线与成交额，采集盘面和新闻快照。来源失败即标记为不可用，不把缺失当成中性。</p></article><article class="flow-step"><b>第 2 步</b><h3>逐条审核情绪</h3><p>当前 Codex 对话逐条审核已采集信息：相关主题、方向、期限、证据与风险。任何漏审，明天都禁止新开仓。</p></article><article class="flow-step"><b>第 3 步</b><h3>检查持仓与核心候选</h3><p>核心持仓只按原退出规则处理；挑战者持仓除原退出规则外，只要当天出现合格核心候选，也在明日开盘让位。</p></article><article class="flow-step"><b>第 4 步</b><h3>核心候选优先</h3><p>先在45只核心中运行完整入场路径；只要有一只核心合格，当天就不选择挑战者。</p></article><article class="flow-step"><b>第 5 步</b><h3>无核心才由挑战者补位</h3><p>核心没有候选时，才从技术合格的挑战者中选一只；最终仍只输出一只ETF或现金。</p></article></div></section><section class="section"><div class="section-title"><h2>什么时候买入、卖出或持有？</h2><span>最多一只 ETF；条件不满足时现金也是明确仓位</span></div><div class="decision-grid"><article class="card decision buy"><b>买入条件</b><h3>必须同时满足全部基础条件</h3><span class="all">全部满足才可买</span><ul><li>上市满 120 个交易日，近 20 日平均成交额不低于 2,000 万元；</li><li>动量总分 = ROC20 × 1.0 + ROC60 × 1.5；核心池前5，挑战者用相对核心的虚拟排名；</li><li>ROC20 与 ROC60 都为正，收盘价在 MA120 上方，且不高于 MA120 的 9%；</li><li>先在核心合格标的中选选择分最高的1只；没有核心候选时，挑战者才可补位；</li><li>若排第 4–5 名且 ROC20 低于 2%，还必须由主题广度、热度加速和正 DDE 占比共同确认；</li><li>当天情绪审核覆盖率必须为 100%。</li></ul></article><article class="card decision sell"><b>卖出条件</b><h3>任一硬/软退出条件触发即卖</h3><span class="any">任一触发就卖</span><ul><li>硬退出：收盘价跌破 MA120；这条不能被新闻或情绪信号延后；</li><li>软退出：ROC20 转负；</li><li>软退出：当前决策排名同时弱于 5 个交易日前和 20 个交易日前；核心排名不受挑战者占位影响；</li><li>已审核且持续的主题热度，只能短暂保护两项软退出，不能覆盖 MA120 破位；</li><li>卖出后同一 ETF 冷却 5 个交易日，不马上追回。</li></ul></article><article class="card decision"><b>持有 / 换仓</b><h3>先卖后定目标，不凭感觉加仓</h3><ul><li>核心持仓没有卖出信号：继续持有，即使出现别的候选；</li><li>挑战者持仓若遇到合格核心候选：次日开盘让位并切回核心；</li><li>原持仓出现卖出信号、没有合格候选：次日开盘卖出后空仓；</li><li>没有持仓且没有候选：继续空仓。</li></ul></article><article class="card decision"><b>情绪模块的边界</b><h3>用于确认，不制造买点</h3><ul><li>盘面和新闻逐条审核后形成主题广度、热度加速、正 DDE 占比等确认信息；</li><li>它能否决弱动量/边缘排名的假启动，并可在强主题中保护软退出；</li><li>它不能让不合格的价格趋势直接买入，不能改变核心优先级，也不能覆盖 MA120 的硬退出；</li><li>任一资讯来源或审核不完整：只允许执行价格卖出，不新开仓。</li></ul></article></div><div class="execution-note"><b>执行时间：</b>T 日收盘后完成上述判断 → T+1 开盘按订单计划一次性执行。普通 ETF 单边成本固定为 0.15%；QDII/溢价敏感 ETF 单边成本固定为 0.30%；每笔最低佣金 5 元。回测与实盘使用同一口径。</div></section>'''
...
def build_strategy_page() -> str:
    ...
    body = f'''
<section class="section"><article class="card strategy-lead"><h2>这套策略每天只回答一个问题：明天开盘持有哪一只 ETF，还是持有现金？</h2><p>它不是每天追排行榜第一名。先检查真实旧仓是否必须卖；只有账户空仓或旧仓触发退出，才在45只核心ETF与6只挑战者中运行入场路径。核心候选拥有买入优先权；挑战者只在核心无候选时补位。</p></article></section>
...
<section class="section"><div class="section-title"><h2>每天按这四步走</h2><span>状态顺序固定，不允许跳步</span></div><div class="decision-map"><article class="card decision-step"><em>01 · 输入</em><h3>冻结价格与全部资讯</h3><p>更新51只ETF日线和成交额；每条资讯都保留哈希并逐条审核。</p><strong>审核不是100% → 禁止新买</strong></article><article class="card decision-step"><em>02 · 旧仓</em><h3>先判断是否必须卖</h3><p>检查MA120、ROC20以及5日/20日决策排名是否同时恶化。</p><strong>未触发 → 原仓继续持有</strong></article><article class="card decision-step"><em>03 · 新仓</em><h3>空仓时才选候选</h3><p>先在核心池中完成筛选；只有核心没有候选时，才检查技术合格的挑战者。</p><strong>挑战者只补核心空档</strong></article><article class="card decision-step"><em>04 · 目标</em><h3>最终只留下一个</h3><p>核心有候选就选核心第一名；否则才选挑战者第一名，无候选持有现金。</p><strong>输出唯一0% / 100%</strong></article></div></section>
<section class="section"><div class="section-title"><h2>筛选漏斗：每一层在做什么</h2><span>先资格，后三条路径并行，最后结合账户状态</span></div><div class="card table-wrap"><table class="simple execution-table"><thead><tr><th>层级</th><th>检查内容</th><th>不通过时</th></tr></thead><tbody><tr><td>冻结母池</td><td>45只核心＋6只挑战者。核心是冠军池，挑战者是空档补位池；名单只在半年或年度审计中调整。</td><td>池外ETF不参与。</td></tr><tr><td>新开仓资格</td><td>上市至少120个交易日；最近20日成交额中位数至少2,000万元。</td><td>不能新买；不合格挑战者也不能占核心名额。</td></tr><tr><td>锚定排名</td><td>核心只在45只内部排名；挑战者计算相对核心池的虚拟排名，但不与合格核心争夺买入权。</td><td>常规路径要求前5；新趋势最多放宽到前15。</td></tr><tr><td>三条技术入场路径</td><td>常规动量、新趋势例外、9%—12%质量延伸并行判断，任一通过即可进入技术候选；核心优先后才形成最终候选。</td><td>三条都不通过则淘汰；技术通过不等于挑战者当天可买。</td></tr><tr><td>账户状态</td><td>核心旧仓未触发卖出时继续持有；挑战者遇到合格核心则让位；空仓时先选核心。</td><td>没有最终候选就持有现金。</td></tr></tbody></table></div></section>
...
<section class="section"><div class="section-title"><h2>最终候选如何排序</h2><span>不同路径使用配置中明确的选择分</span></div><article class="card callout"><h3>常规动量与质量延伸看核心动量分；新趋势看专属分</h3><p>常规动量和质量延伸使用 ROC20 + 1.5×ROC60；新趋势使用 ROC20 + 0.05×R²20。核心池内按正式选择分排序；只有核心没有候选时，挑战者才按自身选择分补位。核心持仓未触发卖出时不换仓，挑战者则必须在核心候选重新出现时让位。</p></article></section>
<section class="section"><div class="section-title"><h2>持有、卖出与换仓</h2><span>先处理旧仓，再决定是否需要新目标</span></div><div class="rule-matrix"><article class="card rule"><b>继续持有</b><h3>没有卖出信号就不动</h3><ul><li>不要求持仓每天仍在前5；</li><li>核心持仓不因另一只ETF排名更高而换仓；</li><li>挑战者持仓遇到合格核心候选时让位；</li><li>上市期与流动性是入场门槛，不是机械卖出条件。</li></ul></article><article class="card rule"><b>硬退出</b><h3>跌破MA120</h3><ul><li>当日收盘跌破MA120即触发；</li><li>下一交易日开盘卖出；</li><li>任何新闻、热点或AI判断都不能保护。</li></ul></article><article class="card rule"><b>软退出</b><h3>ROC20或排名恶化</h3><ul><li>ROC20转负；或</li><li>当前排名同时差于5日前和20日前；</li><li>任一成立即可触发，默认1日确认。</li></ul></article><article class="card rule"><b>热点保护与冷却</b><h3>只保护软退出</h3><ul><li>强势股≥3、热点评分≥0.60、正DDE占比≥50%，可保护软退出2日；</li><li>MA120破位绝不保护；</li><li>卖出后5个交易日内不得重买同一ETF。</li></ul></article></div></section>
