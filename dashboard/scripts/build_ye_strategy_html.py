...
<section class="section" id="etfDashboard"><div class="section-title"><h2>全池 ETF 观察台</h2><span>45只固定池 · 冻结评分与价格同一截止日</span></div><div class="etf-toolbar"><input id="etfSearch" placeholder="搜索代码或名称"><select id="etfCategory"><option value="">全部分类</option></select><select id="etfSort"><option value="rank">按策略排名</option><option value="return_3m">按近3月收益</option><option value="return_1y">按近1年收益</option><option value="momentum_score">按动量评分</option></select><div class="controls" id="etfRanges"><button data-etf-range="3m" class="active">近3个月</button><button data-etf-range="1y">近1年</button></div></div><article class="card etf-detail"><div id="etfDetailHead"></div><div class="etf-stats" id="etfStats"></div><canvas class="etf-trend" id="etfTrendChart"></canvas><div class="tip" id="etfTrendTip">蓝线为收盘价，橙线为 MA120；点击下表任一 ETF 可切换。</div></article><div class="card table-wrap etf-table"><table class="simple"><thead><tr><th>排名 / ETF</th><th>走势</th><th>最新价</th><th>当日</th><th>近3月</th><th>近1年</th><th>动量分</th><th>ROC20</th><th>ROC60</th><th>MA120乖离</th><th>路径 / 结果</th></tr></thead><tbody id="etfRows"></tbody></table></div></section>
<section class="section" id="strategy"><div class="section-title"><h2>策略规则</h2><span>固定45只ETF · 最多持有1只 · 无信号持有现金</span></div><div class="rules">
...
function renderEtfDetail(){const x=ETF.find(v=>v.symbol===etfSymbol)||ETF[0],pass=x.final_entry_pass;document.getElementById('etfDetailHead').innerHTML=`<div class="etf-detail-head"><div><span class="label">${esc(x.category)} · ${esc(x.symbol)}</span><h3>${esc(x.name)}</h3><span class="label">数据截至 ${DATA.etf_dashboard.as_of}</span></div><span class="etf-badge ${pass?'':'fail'}">${pass?'最终合格 · '+esc(x.path):'当日未进入最终候选'}</span></div>`;const stats=[['最新价',x.close.toFixed(3),''],['策略排名','#'+Math.round(x.rank),''],['动量评分',signed(x.momentum_score,2),tone(x.momentum_score)],['近3月',signed(x.return_3m),tone(x.return_3m)],['近1年',signed(x.return_1y),tone(x.return_1y)],['ROC20',signed(x.roc20),tone(x.roc20)],['ROC60',signed(x.roc60),tone(x.roc60)],['MA120乖离',signed(x.ma120_bias),tone(x.ma120_bias)]];document.getElementById('etfStats').innerHTML=stats.map(s=>`<div class="etf-stat"><span class="label">${s[0]}</span><strong class="${s[2]}">${s[1]}</strong></div>`).join('');drawEtfTrend()}
...
function renderEtfRows(){const q=document.getElementById('etfSearch').value.trim().toLowerCase(),cat=document.getElementById('etfCategory').value,sort=document.getElementById('etfSort').value;let items=ETF.filter(x=>(!q||`${x.symbol}${x.name}`.toLowerCase().includes(q))&&(!cat||x.category===cat));items.sort((a,b)=>sort==='rank'?a.rank-b.rank:(b[sort]??-Infinity)-(a[sort]??-Infinity));document.getElementById('etfRows').innerHTML=items.map(x=>`<tr data-symbol="${x.symbol}" class="${x.symbol===etfSymbol?'selected':''}"><td><b>#${Math.round(x.rank)} ${esc(x.name)}</b><br><span class="label">${x.symbol} · ${esc(x.category)}</span></td><td>${sparkline(x)}</td><td>${x.close.toFixed(3)}</td><td class="${tone(x.change_1d)}">${signed(x.change_1d)}</td><td class="${tone(x.return_3m)}">${signed(x.return_3m)}</td><td class="${tone(x.return_1y)}">${signed(x.return_1y)}</td><td>${signed(x.momentum_score,2)}</td><td class="${tone(x.roc20)}">${signed(x.roc20)}</td><td class="${tone(x.roc60)}">${signed(x.roc60)}</td><td class="${tone(x.ma120_bias)}">${signed(x.ma120_bias)}</td><td><span class="status-dot ${x.final_entry_pass?'pass':''}"></span>${x.final_entry_pass?esc(x.path)+' · 最终合格':'未通过'}</td></tr>`).join('')||'<tr><td colspan="11" class="label">没有匹配的 ETF。</td></tr>';document.querySelectorAll('#etfRows tr[data-symbol]').forEach(r=>r.onclick=()=>{etfSymbol=r.dataset.symbol;renderEtfDetail();renderEtfRows()})}
...
STRATEGY_SECTION = r'''<section class="section" id="strategy"><div class="section-title"><h2>策略规则：每日决策流程</h2><span>当天收盘决定目标仓位 · 下一交易日开盘执行</span></div><article class="card daily-intro"><h3>只在收盘后做一次完整判断</h3><p>策略不猜盘中价格，也不在盘中追涨杀跌。每个交易日收盘后，将当天的价格、成交额、盘面强度和新闻快照一次性冻结；根据下列顺序得出“明日持有哪一只 ETF（或空仓）”，下一个交易日开盘按计划执行。</p></article><div class="flow"><article class="flow-step"><b>收盘后 · 第 1 步</b><h3>冻结当天输入</h3><p>更新全部 45 只 ETF 的日线与成交额，采集盘面和新闻快照。来源失败即标记为不可用，不把缺失当成中性。</p></article><article class="flow-step"><b>第 2 步</b><h3>逐条审核情绪</h3><p>当前 Codex 对话逐条审核已采集信息：相关主题、方向、期限、证据与风险。任何漏审，明天都禁止新开仓。</p></article><article class="flow-step"><b>第 3 步</b><h3>先检查现有持仓</h3><p>持仓触发卖出条件就列入明日开盘卖出；未触发则继续持有，不因另一只 ETF 暂时排名更高而频繁换仓。</p></article><article class="flow-step"><b>第 4 步</b><h3>再筛选候选 ETF</h3><p>对全池计算动量评分和资格条件，过滤假启动、弱动量及流动性不足标的，只保留得分最高的合格候选。</p></article><article class="flow-step"><b>第 5 步</b><h3>输出次日动作</h3><p>结果只有四种：买入、卖出、卖出后换入、继续持有/空仓。统一用下一交易日开盘价和固定成本记账。</p></article></div></section><section class="section"><div class="section-title"><h2>什么时候买入、卖出或持有？</h2><span>最多一只 ETF；条件不满足时现金也是明确仓位</span></div><div class="decision-grid"><article class="card decision buy"><b>买入条件</b><h3>必须同时满足全部基础条件</h3><span class="all">全部满足才可买</span><ul><li>上市满 120 个交易日，近 20 日平均成交额不低于 2,000 万元；</li><li>动量总分 = ROC20 × 1.0 + ROC60 × 1.5，位于固定 45 只 ETF 的前 5 名；</li><li>ROC20 与 ROC60 都为正，收盘价在 MA120 上方，且不高于 MA120 的 9%；</li><li>在所有合格标的中，只选总分最高的 1 只；没有合格标的则不买；</li><li>若排第 4–5 名且 ROC20 低于 2%，还必须由主题广度、热度加速和正 DDE 占比共同确认；</li><li>当天情绪审核覆盖率必须为 100%。</li></ul></article><article class="card decision sell"><b>卖出条件</b><h3>任一硬/软退出条件触发即卖</h3><span class="any">任一触发就卖</span><ul><li>硬退出：收盘价跌破 MA120；这条不能被新闻或情绪信号延后；</li><li>软退出：ROC20 转负；</li><li>软退出：当前动量排名同时弱于 5 个交易日前和 20 个交易日前；</li><li>已审核且持续的主题热度，只能短暂保护两项软退出，不能覆盖 MA120 破位；</li><li>卖出后同一 ETF 冷却 5 个交易日，不马上追回。</li></ul></article><article class="card decision"><b>持有 / 换仓</b><h3>先卖后定目标，不凭感觉加仓</h3><ul><li>原持仓没有卖出信号：继续持有，即使出现别的候选；</li><li>原持仓出现卖出信号、同时有新的合格第一名：次日开盘卖出原持仓并买入新标的；</li><li>原持仓出现卖出信号、没有合格候选：次日开盘卖出后空仓；</li><li>没有持仓且没有候选：继续空仓。</li></ul></article><article class="card decision"><b>情绪模块的边界</b><h3>用于确认，不制造买点</h3><ul><li>盘面和新闻逐条审核后形成主题广度、热度加速、正 DDE 占比等确认信息；</li><li>它能否决弱动量/边缘排名的假启动，并可在强主题中保护软退出；</li><li>它不能让不合格的价格趋势直接买入，也不能覆盖 MA120 的硬退出；</li><li>任一资讯来源或审核不完整：只允许执行价格卖出，不新开仓。</li></ul></article></div><div class="execution-note"><b>执行时间：</b>T 日收盘后完成上述判断 → T+1 开盘按订单计划一次性执行。普通 ETF 单边成本固定为 0.15%；QDII/溢价敏感 ETF 单边成本固定为 0.30%；每笔最低佣金 5 元。回测与实盘使用同一口径。</div></section>'''
...
def build_daily_page() -> str:
    ...
    rankings = rankings.loc[rankings["date"].eq(date)].copy()
    ...
    for row in rankings.sort_values("rank").itertuples(index=False):
        ...
        rows.append(
            f"<tr{top}><td>{int(row.rank)}</td><td><b>{html_lib.escape(str(row.name))}</b><br><span class=\"muted\">{row.symbol} · {row.category}</span></td>"
    ...
    overview_paragraphs = [
        f"结论：本次使用{date}收盘数据决定下一交易日动作。实盘账户{account_position_plain}，可用现金{as_float(account.get('available_cash')):,.2f}元，收盘总权益{as_float(account.get('total_equity')):,.2f}元。明日计划为“{action}{target_name}”，目标仓位{target_weight}；{decision_story}",
        f"第一步先查旧仓，不先追排行榜。卖出只看三项：跌破MA120、ROC20转负、当前排名同时差于5日前和20日前。今日检查为：{held_exit_checks}。因此旧仓处理结论明确；价格退出一旦触发，新闻和热点不能推翻。",
        f"第二步筛全池。固定{len(rankings)}只ETF全部计算，{pool_count}只通过上市满120个交易日和20日成交额中位数不低于2,000万元的资格门槛。三条路径并行：常规动量{normal_count}只、新趋势{emerging_count}只、9%—12%质量延伸{extension_count}只，取并集后最终合格{len(candidates)}只。合格只代表进入候选，不代表同时买入。",
        f"第三步核对目标：{target_snapshot}。排名只是起点，还必须同时满足双ROC、MA120位置、乖离上限，以及对应路径的趋势质量或热点条件；任何关键门槛失败都不能入选。",
        f"第四步审核资讯：{reviewed}/{total}条已逐条完成，覆盖率{(reviewed / total if total else 0):.0%}；{target_category}主题为{target_theme['positive']}条正向、{target_theme['negative']}条负向。资讯只确认指定例外和软退出保护，不能凭新闻制造买点，也不能覆盖MA120硬退出。最终状态{status}，{order_text}明日按计划执行，不临时追涨、加仓或改标的。",
    ]
...
<section class="section"><div class="section-title"><h2>45只 ETF 逐层筛选明细</h2><span>“—”表示该路径未通过；最后一列给出主要原因或处理</span></div><div class="card table-wrap"><table class="simple"><thead><tr><th>排名</th><th>ETF</th><th>资格</th><th>动量分</th><th>ROC20</th><th>ROC60</th><th>MA120乖离</th><th>常规</th><th>新趋势</th><th>延伸</th><th>最终结果</th></tr></thead><tbody>{''.join(rows)}</tbody></table></div></section>'''
...
def build_strategy_page() -> str:
    ...
    body = f'''
<section class="section"><article class="card strategy-lead"><h2>这套策略每天只回答一个问题：明天开盘持有哪一只 ETF，还是持有现金？</h2><p>它不是每天追排行榜第一名。先以真实账户检查旧仓是否必须卖；旧仓没有卖出信号就继续持有。只有账户空仓或旧仓触发退出，才在固定45只ETF中运行三条入场路径，并从最终合格者中选一个目标。</p></article></section>
...
<section class="section"><div class="section-title"><h2>每天按这四步走</h2><span>状态顺序固定，不允许跳步</span></div><div class="decision-map"><article class="card decision-step"><em>01 · 输入</em><h3>冻结价格与全部资讯</h3><p>更新45只ETF日线和成交额；每条资讯都保留哈希并逐条审核。</p><strong>审核不是100% → 禁止新买</strong></article><article class="card decision-step"><em>02 · 旧仓</em><h3>先判断是否必须卖</h3><p>检查MA120、ROC20以及5日/20日排名是否同时恶化。</p><strong>未触发 → 原仓继续持有</strong></article><article class="card decision-step"><em>03 · 新仓</em><h3>空仓时才选候选</h3><p>先过上市期和流动性，再让常规、新趋势、质量延伸三条路径并行筛选。</p><strong>三条路径取并集</strong></article><article class="card decision-step"><em>04 · 目标</em><h3>最终只留下一个</h3><p>按正式路径选择分排序；有候选买第一名，无候选持有现金。</p><strong>输出唯一0% / 100%</strong></article></div></section>
<section class="section"><div class="section-title"><h2>筛选漏斗：每一层在做什么</h2><span>先资格，后三条路径并行，最后结合账户状态</span></div><div class="card table-wrap"><table class="simple execution-table"><thead><tr><th>层级</th><th>检查内容</th><th>不通过时</th></tr></thead><tbody><tr><td>固定母池</td><td>固定45只、12类ETF。每天全部计算，不临时追热点加品种。</td><td>池外ETF不参与。</td></tr><tr><td>新开仓资格</td><td>上市至少120个交易日；最近20日成交额中位数至少2,000万元。</td><td>不能新买；该条件本身不强制卖出旧仓。</td></tr><tr><td>核心排名</td><td>动量分 = ROC20 + 1.5×ROC60。MA120下方的ETF在排名分中扣1000个百分点。</td><td>常规路径要求排名前5；新趋势最多放宽到前15。</td></tr><tr><td>三条入场路径</td><td>常规动量、新趋势例外、9%—12%质量延伸并行判断，任一通过即可进入最终候选。</td><td>三条都不通过则淘汰。</td></tr><tr><td>账户状态</td><td>旧仓未触发卖出时继续持有；空仓或已退出时，才从最终候选中选路径选择分最高者。</td><td>没有最终候选就持有现金。</td></tr></tbody></table></div></section>
<section class="section"><div class="section-title"><h2>ye 当前完整规则：三条入场路径</h2><span>它们是三种不同机会，不是逐级放宽</span></div><div class="path-grid"><article class="card path-card"><span class="label">路径 A</span><h3>常规动量</h3><p>排名≤5；ROC20&gt;0、ROC60&gt;0；价格在MA120上方；MA120乖离≤9%。其中排名4—5且ROC20&lt;2%的弱边缘候选，还要满足强势股≥3、数量加速≥0、正DDE占比≥50%。</p></article><article class="card path-card"><span class="label">路径 B</span><h3>新趋势例外</h3><p>排名≤15；ROC20≥3%；ROC60在-8%至0%；MA120乖离在-3%至9%；R²20≥0.70、效率≥0.20，并满足强势股≥3、热点评分≥0.45、数量加速≥0.25、正DDE占比≥50%。触发记忆3日。</p></article><article class="card path-card"><span class="label">路径 C</span><h3>9%—12%质量延伸</h3><p>排名≤5、双ROC为正、MA120上方；乖离&gt;9%且≤12%；R²20≥0.75、效率≥0.40、ROC5≥-1%，并满足强势股≥2、热点评分≥0.45、数量加速≥0。</p></article></div></section>
<section class="section"><div class="section-title"><h2>最终候选如何排序</h2><span>不同路径使用配置中明确的选择分</span></div><article class="card callout"><h3>常规动量与质量延伸看核心动量分；新趋势看专属分</h3><p>常规动量和质量延伸使用 ROC20 + 1.5×ROC60；新趋势使用 ROC20 + 0.05×R²20。三条路径通过者合并后按对应的正式选择分排序。已有持仓未触发卖出时，不因为新候选分数更高而换仓。</p></article></section>
<section class="section"><div class="section-title"><h2>持有、卖出与换仓</h2><span>先处理旧仓，再决定是否需要新目标</span></div><div class="rule-matrix"><article class="card rule"><b>继续持有</b><h3>没有卖出信号就不动</h3><ul><li>不要求持仓每天仍在前5；</li><li>不因另一只ETF排名更高而换仓；</li><li>上市期与流动性是入场门槛，不是机械卖出条件。</li></ul></article><article class="card rule"><b>硬退出</b><h3>跌破MA120</h3><ul><li>当日收盘跌破MA120即触发；</li><li>下一交易日开盘卖出；</li><li>任何新闻、热点或AI判断都不能保护。</li></ul></article><article class="card rule"><b>软退出</b><h3>ROC20或排名恶化</h3><ul><li>ROC20转负；或</li><li>当前排名同时差于5日前和20日前；</li><li>任一成立即可触发，默认1日确认。</li></ul></article><article class="card rule"><b>热点保护与冷却</b><h3>只保护软退出</h3><ul><li>强势股≥3、热点评分≥0.60、正DDE占比≥50%，可保护软退出2日；</li><li>MA120破位绝不保护；</li><li>卖出后5个交易日内不得重买同一ETF。</li></ul></article></div></section>
<section class="section"><div class="section-title"><h2>AI、资讯和市场判断的边界</h2><span>这里最容易被误解</span></div><div class="truth-grid"><article class="card truth"><span class="num">资讯完整性</span><h3>全部行必须审核</h3><p>漏审、重复、来源失败或覆盖不足100%，一律禁止新开仓；已有仓位的价格卖出仍执行。</p></article><article class="card truth"><span class="num">AI的作用</span><h3>确认指定路径，不凭新闻造趋势</h3><p>热点指标用于弱边缘、新趋势、质量延伸和软退出保护。正式规则没有统一“负面风险阈值”否决所有常规买点。</p></article><article class="card truth"><span class="num">全市场趋势</span><h3>没有独立大盘开关</h3><p>策略判断单只ETF趋势和主题热点；不要求沪深300站上均线，也没有risk-on/risk-off或防守ETF切换。</p></article></div></section>
...
<section class="section"><div class="section-title"><h2>ye 与 etfwin 规则对照</h2><span>etfwin 为公开指南的本地量化代理，仅作对照</span></div><div class="card table-wrap"><table class="simple execution-table"><thead><tr><th>项目</th><th>ye 策略</th><th>etfwin 参考策略</th></tr></thead><tbody><tr><td>ETF 池</td><td>固定 45 只、12 类；带上市期与流动性门槛</td><td>公开指南对应的固定 20 只参考池</td></tr><tr><td>核心评分</td><td>ROC20 + 1.5 × ROC60</td><td>ROC20 + 1.5 × ROC60</td></tr><tr><td>通常买入</td><td>前 5、双 ROC 为正、MA120 上方、乖离≤9%</td><td>前 5、双 ROC 为正、MA120 上方、乖离≤15%</td></tr><tr><td>增强判断</td><td>主题广度、AI 弱边缘确认、新趋势、9%–12%质量延伸、热点软退出保护</td><td>无本地增强模块</td></tr><tr><td>卖出</td><td>MA120破位 / ROC20转负 / 5日与20日排名双降；同标的冷却5日</td><td>MA120破位 / ROC20转负 / 5日与20日排名双降</td></tr><tr><td>仓位与执行</td><td>一只 ETF 或现金；T+1 开盘</td><td>同样按一只 ETF 或现金、T+1 开盘重建对照</td></tr></tbody></table></div></section>
