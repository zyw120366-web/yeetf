...
function renderEtfDetail(){const x=ETF.find(v=>v.symbol===etfSymbol)||ETF[0],pass=x.final_entry_pass,role=x.pool_role==='challenger'?'挑战者':'核心';document.getElementById('etfDetailHead').innerHTML=`<div class="etf-detail-head"><div><span class="label">${esc(x.category)} · ${esc(x.symbol)} · ${role}</span><h3>${esc(x.name)}</h3><span class="label">数据截至 ${DATA.etf_dashboard.as_of}</span></div><span class="etf-badge ${pass?'':'fail'}">${pass?'最终合格 · '+esc(x.path):'当日未进入最终候选'}</span></div>`;const stats=[['最新价',x.close.toFixed(3),''],['决策排名','#'+Math.round(x.rank),''],['动量评分',signed(x.momentum_score,2),tone(x.momentum_score)],['近3月',signed(x.return_3m),tone(x.return_3m)],['近1年',signed(x.return_1y),tone(x.return_1y)],['ROC20',signed(x.roc20),tone(x.roc20)],['ROC60',signed(x.roc60),tone(x.roc60)],['MA120乖离',signed(x.ma120_bias),tone(x.ma120_bias)]];document.getElementById('etfStats').innerHTML=stats.map(s=>`<div class="etf-stat"><span class="label">${s[0]}</span><strong class="${s[2]}">${s[1]}</strong></div>`).join('');drawEtfTrend()}
...
function renderEtfRows(){const q=document.getElementById('etfSearch').value.trim().toLowerCase(),cat=document.getElementById('etfCategory').value,sort=document.getElementById('etfSort').value;let items=ETF.filter(x=>(!q||`${x.symbol}${x.name}`.toLowerCase().includes(q))&&(!cat||x.category===cat));items.sort((a,b)=>sort==='rank'?a.rank-b.rank:(b[sort]??-Infinity)-(a[sort]??-Infinity));document.getElementById('etfRows').innerHTML=items.map(x=>`<tr data-symbol="${x.symbol}" class="${x.symbol===etfSymbol?'selected':''}"><td><b>#${Math.round(x.rank)} ${esc(x.name)}</b><br><span class="label">${x.symbol} · ${esc(x.category)} · ${x.pool_role==='challenger'?'挑战者':'核心'}</span></td><td>${sparkline(x)}</td><td>${x.close.toFixed(3)}</td><td class="${tone(x.change_1d)}">${signed(x.change_1d)}</td><td class="${tone(x.return_3m)}">${signed(x.return_3m)}</td><td class="${tone(x.return_1y)}">${signed(x.return_1y)}</td><td>${signed(x.momentum_score,2)}</td><td class="${tone(x.roc20)}">${signed(x.roc20)}</td><td class="${tone(x.roc60)}">${signed(x.roc60)}</td><td class="${tone(x.ma120_bias)}">${signed(x.ma120_bias)}</td><td><span class="status-dot ${x.final_entry_pass?'pass':''}"></span>${x.final_entry_pass?esc(x.path)+' · 最终合格':'未通过'}</td></tr>`).join('')||'<tr><td colspan="11" class="label">没有匹配的 ETF。</td></tr>';document.querySelectorAll('#etfRows tr[data-symbol]').forEach(r=>r.onclick=()=>{etfSymbol=r.dataset.symbol;renderEtfDetail();renderEtfRows()})}
...
STRATEGY_SECTION = r'''<section class="section" id="strategy"><div class="section-title"><h2>策略规则：每日决策流程</h2><span>当天收盘决定目标仓位 · 下一交易日开盘执行</span></div><article class="card daily-intro"><h3>先保护原有标尺，再扩大题材覆盖</h3><p>全池由45只核心ETF和6只挑战者组成。核心池独立形成原有排名与类别宽度；挑战者只有完整满足上市、流动性、价格趋势和AI确认路径后，才按相对核心池的虚拟排名竞争。尚不可买的挑战者不会占据核心前5，也不会改变核心持仓的排名退出。</p></article><div class="flow"><article class="flow-step"><b>收盘后 · 第 1 步</b><h3>冻结当天输入</h3><p>更新全部51只ETF的日线与成交额，采集盘面和新闻快照。来源失败即标记为不可用，不把缺失当成中性。</p></article><article class="flow-step"><b>第 2 步</b><h3>逐条审核情绪</h3><p>当前 Codex 对话逐条审核已采集信息：相关主题、方向、期限、证据与风险。任何漏审，明天都禁止新开仓。</p></article><article class="flow-step"><b>第 3 步</b><h3>先检查现有持仓</h3><p>持仓触发卖出条件就列入明日开盘卖出；未触发则继续持有，不因另一只 ETF 暂时排名更高而频繁换仓。</p></article><article class="flow-step"><b>第 4 步</b><h3>再筛选候选 ETF</h3><p>核心ETF按核心排名筛选；挑战者先过完整资格，再计算相对核心的虚拟排名，最终候选按同一选择分竞争。</p></article><article class="flow-step"><b>第 5 步</b><h3>输出次日动作</h3><p>结果只有四种：买入、卖出、卖出后换入、继续持有/空仓。统一用下一交易日开盘价和固定成本记账。</p></article></div></section><section class="section"><div class="section-title"><h2>什么时候买入、卖出或持有？</h2><span>最多一只 ETF；条件不满足时现金也是明确仓位</span></div><div class="decision-grid"><article class="card decision buy"><b>买入条件</b><h3>必须同时满足全部基础条件</h3><span class="all">全部满足才可买</span><ul><li>上市满 120 个交易日，近 20 日平均成交额不低于 2,000 万元；</li><li>动量总分 = ROC20 × 1.0 + ROC60 × 1.5；核心池前5，挑战者用相对核心的虚拟排名；</li><li>ROC20 与 ROC60 都为正，收盘价在 MA120 上方，且不高于 MA120 的 9%；</li><li>在所有合格标的中，只选选择分最高的 1 只；没有合格标的则不买；</li><li>若排第 4–5 名且 ROC20 低于 2%，还必须由主题广度、热度加速和正 DDE 占比共同确认；</li><li>当天情绪审核覆盖率必须为 100%。</li></ul></article><article class="card decision sell"><b>卖出条件</b><h3>任一硬/软退出条件触发即卖</h3><span class="any">任一触发就卖</span><ul><li>硬退出：收盘价跌破 MA120；这条不能被新闻或情绪信号延后；</li><li>软退出：ROC20 转负；</li><li>软退出：当前决策排名同时弱于 5 个交易日前和 20 个交易日前；核心排名不受挑战者占位影响；</li><li>已审核且持续的主题热度，只能短暂保护两项软退出，不能覆盖 MA120 破位；</li><li>卖出后同一 ETF 冷却 5 个交易日，不马上追回。</li></ul></article><article class="card decision"><b>持有 / 换仓</b><h3>先卖后定目标，不凭感觉加仓</h3><ul><li>原持仓没有卖出信号：继续持有，即使出现别的候选；</li><li>原持仓出现卖出信号、同时有新的合格第一名：次日开盘卖出原持仓并买入新标的；</li><li>原持仓出现卖出信号、没有合格候选：次日开盘卖出后空仓；</li><li>没有持仓且没有候选：继续空仓。</li></ul></article><article class="card decision"><b>情绪模块的边界</b><h3>用于确认，不制造买点</h3><ul><li>盘面和新闻逐条审核后形成主题广度、热度加速、正 DDE 占比等确认信息；</li><li>它能否决弱动量/边缘排名的假启动，并可在强主题中保护软退出；</li><li>它不能让不合格的价格趋势直接买入，也不能覆盖 MA120 的硬退出；</li><li>任一资讯来源或审核不完整：只允许执行价格卖出，不新开仓。</li></ul></article></div><div class="execution-note"><b>执行时间：</b>T 日收盘后完成上述判断 → T+1 开盘按订单计划一次性执行。普通 ETF 单边成本固定为 0.15%；QDII/溢价敏感 ETF 单边成本固定为 0.30%；每笔最低佣金 5 元。回测与实盘使用同一口径。</div></section>'''
...
<section class="section"><div class="section-title"><h2>今日筛选漏斗</h2><span>不仅看数量，也直接看留下了谁</span></div><div class="filter-strip"><article class="card filter-stage"><small>冻结母池</small><strong>{len(rankings)} 只</strong><p>45只核心＋{len(rankings) - 45}只挑战者；挑战者不占核心名额。</p></article><article class="card filter-stage"><small>上市期与流动性</small><strong>{pool_count} 只</strong><p>{pool_count}只通过基础资格。</p></article><article class="card filter-stage"><small>最终合格</small><strong>{len(candidates)} 只</strong><p>{html_lib.escape(candidate_names)}</p></article><article class="card filter-stage"><small>明日实际目标</small><strong>{1 if target_symbol else 0} 只</strong><p>{html_lib.escape(str(target_name))}{'（继续持有）' if current_symbol == target_symbol and target_symbol else ''}</p></article></div></section>
...
<section class="section"><div class="section-title"><h2>{len(rankings)}只 ETF 逐层筛选明细</h2><span>排名为核心排名或挑战者相对核心的虚拟排名</span></div><div class="card table-wrap"><table class="simple"><thead><tr><th>决策排名</th><th>ETF / 池角色</th><th>资格</th><th>动量分</th><th>ROC20</th><th>ROC60</th><th>MA120乖离</th><th>常规</th><th>新趋势</th><th>延伸</th><th>最终结果</th></tr></thead><tbody>{''.join(rows)}</tbody></table></div></section>'''
...
def build_strategy_page() -> str:
    ...
    body = f'''
<section class="section"><article class="card strategy-lead"><h2>这套策略每天只回答一个问题：明天开盘持有哪一只 ETF，还是持有现金？</h2><p>它不是每天追排行榜第一名。先检查真实旧仓是否必须卖；只有账户空仓或旧仓触发退出，才在45只核心ETF与6只挑战者中运行入场路径。挑战者尚不可买时不会挤占核心排名。</p></article></section>
...
<section class="section"><div class="section-title"><h2>每天按这四步走</h2><span>状态顺序固定，不允许跳步</span></div><div class="decision-map"><article class="card decision-step"><em>01 · 输入</em><h3>冻结价格与全部资讯</h3><p>更新51只ETF日线和成交额；每条资讯都保留哈希并逐条审核。</p><strong>审核不是100% → 禁止新买</strong></article><article class="card decision-step"><em>02 · 旧仓</em><h3>先判断是否必须卖</h3><p>检查MA120、ROC20以及5日/20日决策排名是否同时恶化。</p><strong>未触发 → 原仓继续持有</strong></article><article class="card decision-step"><em>03 · 新仓</em><h3>空仓时才选候选</h3><p>核心保留原排名；挑战者先完整合格，再按相对核心的虚拟排名竞争。</p><strong>不合格挑战者零干扰</strong></article><article class="card decision-step"><em>04 · 目标</em><h3>最终只留下一个</h3><p>按正式路径选择分排序；有候选买第一名，无候选持有现金。</p><strong>输出唯一0% / 100%</strong></article></div></section>
<section class="section"><div class="section-title"><h2>筛选漏斗：每一层在做什么</h2><span>先资格，后三条路径并行，最后结合账户状态</span></div><div class="card table-wrap"><table class="simple execution-table"><thead><tr><th>层级</th><th>检查内容</th><th>不通过时</th></tr></thead><tbody><tr><td>冻结母池</td><td>45只核心＋6只挑战者。名单只在半年或年度审计中调整。</td><td>池外ETF不参与。</td></tr><tr><td>新开仓资格</td><td>上市至少120个交易日；最近20日成交额中位数至少2,000万元。</td><td>不能新买；不合格挑战者也不能占核心名额。</td></tr><tr><td>锚定排名</td><td>核心只在45只内部排名；合格挑战者计算相对核心池的虚拟排名。</td><td>常规路径要求前5；新趋势最多放宽到前15。</td></tr><tr><td>三条入场路径</td><td>常规动量、新趋势例外、9%—12%质量延伸并行判断，任一通过即可进入最终候选。</td><td>三条都不通过则淘汰。</td></tr><tr><td>账户状态</td><td>旧仓未触发卖出时继续持有；空仓或已退出时，才从最终候选中选路径选择分最高者。</td><td>没有最终候选就持有现金。</td></tr></tbody></table></div></section>
...
<section class="section"><div class="section-title"><h2>ye 与 etfwin 规则对照</h2><span>etfwin 为公开指南的本地量化代理，仅作对照</span></div><div class="card table-wrap"><table class="simple execution-table"><thead><tr><th>项目</th><th>ye 策略</th><th>etfwin 参考策略</th></tr></thead><tbody><tr><td>ETF 池</td><td>45只核心＋6只挑战者；锚定排名与入场隔离</td><td>公开指南对应的固定 20 只参考池</td></tr><tr><td>核心评分</td><td>ROC20 + 1.5 × ROC60</td><td>ROC20 + 1.5 × ROC60</td></tr><tr><td>通常买入</td><td>前 5、双 ROC 为正、MA120 上方、乖离≤9%</td><td>前 5、双 ROC 为正、MA120 上方、乖离≤15%</td></tr><tr><td>增强判断</td><td>主题广度、AI 弱边缘确认、新趋势、9%–12%质量延伸、热点软退出保护</td><td>无本地增强模块</td></tr><tr><td>卖出</td><td>MA120破位 / ROC20转负 / 5日与20日决策排名双降；同标的冷却5日</td><td>MA120破位 / ROC20转负 / 5日与20日排名双降</td></tr><tr><td>仓位与执行</td><td>一只 ETF 或现金；T+1 开盘</td><td>同样按一只 ETF 或现金、T+1 开盘重建对照</td></tr></tbody></table></div></section>
