...
**日报链接是必交付物**：稳定入口为 `outputs/ETF轮动策略_今日日报.html`。普通对话使用无空格的绝对 Markdown 文件链接，例如 `[打开今日日报](/绝对路径/outputs/ETF轮动策略_今日日报.html)`；不得只给相对路径或纯文本路径。自动任务的正文只保留简短交易结论和“日报已生成”，不堆叠文件路径、JSON、XML字段或技术审计信息；日报入口由运行界面打开。其余 HTML 和运行卡仅在用户索要时给出链接；审核细节保留在日报和审计文件中。
